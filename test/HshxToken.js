...
contract('HshxToken', function(accounts) { 
	var tokenInstance;
	it('initialises the contract with the correct values', function(){
		return HshxToken.deployed().then(function(instance) {
			tokenInstance = instance;
			return tokenInstance.name();
		}).then(function(name){
			assert.equal(name, 'HshxToken', 'has the correct name');
			return tokenInstance.ticker();
		}).then(function(ticker) {
			assert.equal(ticker, 'HSH', 'has the correct ticker');
		});	
	})
